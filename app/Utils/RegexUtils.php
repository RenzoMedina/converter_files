...
class RegexUtils
{
    ...
    public const EXTRACT_OPTIONS_MULTICHOICES = '/Alternativas\s*(.*?)(?=Indicador\s+de\s+evaluación|^Respuesta\s+correcta)/ms';
    ...
    public const CLEAN_FULL_TEXT = '/(N°\s*de\s*pregunta:?\s*\d+|Pregunta\s*\d+)/i';
    ...
    public const CLEAN_ANSWER = '/^Respuesta\s*correcta\s*([a-e]|verdadero|falso)\s*$/mi';
}